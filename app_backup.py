...
app.config['DEBUG'] = True  # Development modunda otomatik yenileme
...
FIRMA_KAYIT_FILE = 'firma_kayit.json'
SAHA_OLC_FILE = 'saha_olc.json'
BACA_BILGILERI_FILE = 'baca_bilgileri.json'
PARAMETRE_OLCUM_FILE = 'parametre_olcum.json'
TEKLIF_FILE = 'teklif.json'
...
def load_used_teklif_numbers():
    ...
    try:
        used_numbers_file = 'used_teklif_numbers.json'
...
try:
    ...
    try:
        ...
        # Gelen veriyi yerel olarak kaydet
        with open('il-ilce.json', 'w', encoding='utf-8') as f:
            ...
            try:
                ...
                if tarih_str:
                    ...
                    if baslangic <= olcum_tarih <= bitis:
                        ...
                        if kk_deger is not None and kk_deger != 0:
                            try:
...
def api_kk_rapor_olustur():
    ...
    try:
        ...
        for param in parameters:
            if param.get('Metot') == 'So2' and param.get('Parametre Adı') == 'Yg':
                try:
                    kk_degerleri['SO2'] = {
                        'kk': float(param.get('KK', 500)),
                        'minus_3s': float(param.get('-3S', 400)),
                        'minus_2s': float(param.get('-2S', 450)),
                        'plus_2s': float(param.get('+2S', 550)),
                        'plus_3s': float(param.get('+3S', 600))
                    }
                except (ValueError, TypeError):
                    # Eğer değerler alınamazsa varsayılan değerleri kullan
                    kk_degerleri['SO2'] = {'kk': 500, 'minus_3s': 400, 'minus_2s': 450, 'plus_2s': 550, 'plus_3s': 600}
                break
        
        # Eğer SO2 değerleri bulunamadıysa varsayılan değerleri ekle
        if 'SO2' not in kk_degerleri:
            kk_degerleri['SO2'] = {'kk': 500, 'minus_3s': 400, 'minus_2s': 450, 'plus_2s': 550, 'plus_3s': 600}
...
def create_kk_excel_report(parametre, gercek_veriler, kk, minus_3s, minus_2s, plus_2s, plus_3s, ortalama, standart_sapma, min_deger, max_deger, tarih_baslangic, tarih_bitis):
    """KK Excel raporu oluştur - Word formatında"""
    try:
        ...
        # Başlık
        ws['A1'] = f'{parametre} KALİTE KONTROL RAPORU'
        ws['A1'].font = Font(size=24, bold=True)
        ws.merge_cells('A1:G1')
        ws['A1'].alignment = Alignment(horizontal='center')
        
        # Grafik oluştur ve Excel'e ekle
        try:
            # Grafik oluştur
            from datetime import datetime, timedelta
            ...
            print(f"Excel grafik ekleme hatası: {e}")
            ws['A3'] = "Grafik oluşturulamadı."
        
        # Ölçüm verileri tablosu (grafiğin altında)
        data_start = 25
        ws[f'A{data_start}'] = 'Ölçüm Verileri'
        ws[f'A{data_start}'].font = Font(size=16, bold=True)
        
        if gercek_veriler:
            headers = ['Sıra', 'Tarih', 'Firma', 'Kod', 'Baca', 'Personel', 'Değer']
            for i, header in enumerate(headers):
                cell = ws[f'{chr(65+i)}{data_start + 2}']
                cell.value = header
            ...
            for i, veri in enumerate(gercek_veriler, 1):
                ...
                try:
                    ...
                    ws[f'B{row}'] = formatted_tarih
                except:
                    ws[f'B{row}'] = veri['tarih']
                
                ws[f'C{row}'] = veri['firma']
                ws[f'D{row}'] = veri['kod']
                ws[f'E{row}'] = veri['cihaz']
                ws[f'F{row}'] = veri['personel']
                ws[f'G{row}'] = veri['deger']
        else:
            ws[f'A{data_start + 2}'] = 'Bu tarih aralığında ölçüm verisi bulunamadı'
        
        # Sütun genişliklerini ayarla - Sıra No dar, diğerleri geniş
        ws.column_dimensions['A'].width = 8   # Sıra No - dar
        ws.column_dimensions['B'].width = 12  # Tarih
        ws.column_dimensions['C'].width = 18  # Firma
        ws.column_dimensions['D'].width = 18  # Kod
        ws.column_dimensions['E'].width = 18  # Baca
        ws.column_dimensions['F'].width = 15  # Personel
        ws.column_dimensions['G'].width = 12  # Değer
        ...
        try:
            ...
            response.headers['Content-Disposition'] = f'attachment; filename="KK_Rapor_{parametre}_{tarih_baslangic}_{tarih_bitis}.xlsx"'
...
def create_kk_word_report(parametre, gercek_veriler, kk, minus_3s, minus_2s, plus_2s, plus_3s, ortalama, standart_sapma, min_deger, max_deger, tarih_baslangic, tarih_bitis):
    ...
    try:
        ...
        # Başlık
        title = doc.add_heading('KALİTE KONTROL RAPORU', 0)
        ...
        # Grafik ekle
        doc.add_paragraph()
        doc.add_heading('Kalite Kontrol Grafiği', level=1)
        ...
        if gercek_veriler:
            ...
            # Tablo genişliklerini ayarla - Sıra No dar, diğerleri geniş
            data_table.columns[0].width = Inches(0.5)   # Sıra No - dar
            data_table.columns[1].width = Inches(1.0)   # Tarih
            data_table.columns[2].width = Inches(1.5)   # Firma
            ...
            for i, veri in enumerate(gercek_veriler, 1):
                ...
                try:
                    ...
                    row_cells[1].text = veri['tarih']
                
                row_cells[2].text = veri['firma']
                row_cells[3].text = veri['kod']
                row_cells[4].text = veri['cihaz']
                row_cells[5].text = veri['personel']
                row_cells[6].text = str(veri['deger'])
        else:
...
def load_parametre_sahabil():
    ...
    PARAMETRE_SAHABIL_FILE = 'parametre_sahabil.json'
...
def load_parametre_fields():
    ...
    try:
        if not os.path.exists('parametre_fields.json'):
            return {}
        with open('parametre_fields.json', 'r', encoding='utf-8') as f:
            return json.load(f)
    except Exception as e:
...
def save_parametre_fields(parametre_fields_data):
    ...
    try:
        with open('parametre_fields.json', 'w', encoding='utf-8') as f:
...
if __name__ == '__main__':
    ...
    # Geliştirme modunda otomatik reload açık
